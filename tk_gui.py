...
class App:
    ...
    def start_download(self):
        ...
        if platform == "tiktok":
            ...
            # Always attempt cookies — _cookies_opt() tries in order:
            #   1. youtube_cookies.txt (exported Netscape cookies)
            #   2. Live browser extraction (chrome/edge/firefox/brave…)
            #   3. No cookies (public videos only, may fail on bot-detection)
            # iOS/android/mweb extractor_args also set in _build_ydl_opts
            # as a secondary bypass; cookies remain the primary auth method.
            use_cookies = True
